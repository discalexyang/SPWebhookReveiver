...
using SPWebhookReveiver.Commons;



namespace SPWebhookReveiver.Controllers
{
                ...
                if (!string.IsNullOrEmpty(clientStateHeaderValue) && clientStateHeaderValue.Equals(webhookClientState))
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(requestContent))
                        {
                            ...
                            var objNotifications = JsonConvert.DeserializeObject<SPWebhookContent>(requestContent);
                            if (notification != null)
                            {
                                ChangeManager changeManager = new ChangeManager();

                                changeManager.AddNotificationToQueue(Microsoft.Azure.CloudConfigurationManager.GetSetting("storageConnectionString"), notification);
                                ...
                                Task.Factory.StartNew(() =>
                                {
                                    ...
                                    traceWriter.Trace(Request, "SPWebhook Notification",
                                        TraceLevel.Info, string.Format("ExpirationDateTime: {0}", notification.ExpirationDate));
